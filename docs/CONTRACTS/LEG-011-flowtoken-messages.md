...
- **Composite (branching):** a composite class on receiving its request does not
  ...
  (+1), and assigning each branch an autogenerated `branch_id` (distinct per
  branch at this level, constant along the branch's sequence, carried on the
  message). Branches return to the composite's **gathering queue** via their
  `end_of_level_queue`; the returning result's `branch_id` names the join slot.
