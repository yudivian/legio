...
identity: autogenerated by the parallel at fan-out, constant along the branch's
sequence, distinct per branch at the same level of the same task, and preserved
through the branch's execution; empty for non-branch messages), `message_type`
...
- **Parallel (branching)**: on receiving a request the parallel does not advance
  ...
  and an autogenerated `branch_id` (distinct per branch at this level); on fan-in
  ...
  parent-assigned id). On fan-in completion it decrements `level` (−1) and resumes its
