...
- Fan-in identity: dedupe per **(parallel, task, branch slot)**. The **task id
  ...
  **`branch_id`** (a Schema 2 token field) **autogenerated by the parallel at
  fan-out** and **preserved unchanged through the branch's whole execution**
  (base route + sequence re-seed + parallel resume); the result's `branch_id`
  names the branch slot. `branch_id` is constant along a branch's sequence and
  **distinct per branch at the same level of the same task** — so two branches
  that are the very same class/sequence still occupy two distinct join slots of
  that one task. It is generated by the submit/flow-starter and by the parallel
  only. Because `branch_id` is preserved, a **composite/multi-step branch** —
  whose returned `level_route` expands to its own internal steps — still slots
  under the parent-assigned id (its `level_route[0]` would be the branch's
  first *step*, not the branch slot). The join counts slots until all of the
  parallel's branches for the task are present. Join waits (in the bookkeeping,
