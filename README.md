...
- R-2 (Walking skeleton): native beaver substrate (queues/dicts/locks spoken
  ...
  (LEG-011), tool registry (LEG-013), patterns loader + output schema
  (LEG-010/021), ToolAgent (LEG-022), uniform AgentBase.run loop (LEG-023),
  mini-manager + client pseudo-agent (LEG-014/024), federation + naming + errors
  + security (LEG-015/016/017), REST worker surface (LEG-025), E2E `transform`
  example (LEG-026), auth middleware (LEG-027) — **shipped** on a
  board-backed, decoupled polling base.
- R-3 (Atomics): LinguisticAgent (LEG-030) with lingo's `MockLLM` producing a
  ...
  example over the authed REST surface on standing, independent agents
  (LEG-032) — **shipped**.

On the roadmap: R-4 (composites), R-5 (token), R-6 (resilience),
