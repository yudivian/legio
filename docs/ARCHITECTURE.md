...
slot identity: autogenerated by the composite at fan-out, constant along the
branch's sequence, distinct per branch at the same level of the same task, and
preserved through the branch's execution; empty for non-branch messages),
...
5. Each branch returns through its `end_of_level_queue` (the composite's gathering
   ...
   `state:<composite>`), builds the branch results into its payload and
   resumes its level.
