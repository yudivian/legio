...
composite's gathering queue for branches), ``level`` (branch depth, starts at 1),
``launcher_class`` (informational), ``task_id``, and ``branch_id`` (a composite's
branch slot identity: autogenerated by the composite at fan-out, or by the submit
for the root step — ARCHITECTURE §3, never empty), constant along the branch's
sequence, distinct per branch at the same level of the same task, and preserved
through the branch's whole execution). There is no ``next_queue`` /
