...
(O(1) fetch; no child→parent scan). On all-complete it builds its payload from
the branch results (H3 building the payload), decrements ``level`` (−1) and
...
async def fan_out_and_collect(
    *, db: AsyncBeaverDB, par: CompositeAgent, request: ExecutionRequestMessage
...
async def test_parallel_resume_preserves_its_own_branch_id(
    ...
    for child, route in zip(children, par._branches):
        branch_class = route[0][0]
        await beaver_db.queue(queue_key("par")).put(
            child_result(
                branch_id=child.branch_id,
