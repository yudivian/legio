...
| `branch_id` | str, a composite's branch slot identity: **autogenerated** (a unique, random identifier — no parent-derived naming) to avoid unexpected name collisions, assigned by the **submit** at flow birth (the root branch) and by each **composite** at fan-out (its child branches); **every message of the flow carries one** (never empty); constant along the branch's sequence, distinct between branches, preserved unchanged through the branch's whole execution, and used by the fan-in as the join slot. |
